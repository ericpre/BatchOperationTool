...
def add_jupyter_here():
    ...
    for env in ('qtconsole', 'notebook'):
        ...
        key = winreg.CreateKey(
            winreg.HKEY_CLASSES_ROOT,
            r'Directory\shell\Batch_%s_operation_%s_tool' %
            env)
        winreg.SetValueEx(
            key,
            "",
        ...
        key = winreg.CreateKey(
            winreg.HKEY_CLASSES_ROOT,
            r'Directory\shell\Batch_%s_operation_%s_tool\Command' %
            env)
        winreg.SetValueEx(
            key,
            "",
        ...
        key = winreg.CreateKey(
            winreg.HKEY_CLASSES_ROOT,
            r'Directory\Background\shell\Batch_%s_operation_%s_tool' %
            env)
        winreg.SetValueEx(
            key,
            "",
        ...
        key = winreg.CreateKey(
            winreg.HKEY_CLASSES_ROOT,
            r'Directory\Background\shell\Batch_%s_operation_%s_tool\Command' %
            env)
        winreg.SetValueEx(key, "", 0, winreg.REG_EXPAND_SZ, script)
        key.Close()

        print("Batch operation tool %s here context menu entry created." % env)